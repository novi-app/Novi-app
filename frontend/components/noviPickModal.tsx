  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col bg-cream transition-transform duration-500 ease-out ${visible ? "translate-y-0" : "translate-y-full"}`}>
      ...
      >
        <button
          onClick={handleClose}
          className="w-8 h-8 flex items-center justify-center rounded-full bg-white mb-4"
        >
          <svg className="w-4 h-4 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <h1 className="text-2xl font-bold text-white mb-1">One place for you</h1>
      </div>

      {/* Scrollable content */}
      <div className="flex-1 overflow-y-auto px-8 pt-4 pb-4" style={{ scrollbarWidth: "none" }}>

        <p className="text-black font-medium text-md pb-4">No scrolling required. Novi thinks this is your best next move.</p>
      ...
      {/* Footer buttons */}
      <div className="flex gap-3 px-6 pb-8 pt-4 border-t border-gray-200">
        <button
          onClick={onDetails}
          className="flex-1 py-3 rounded-full border-2 border-primary text-primary font-semibold active:scale-[0.98] transition-transform"
        >
          Details
        </button>
        <button
          onClick={onDirections}
          className="flex-1 py-3 rounded-full bg-primary text-white font-semibold active:scale-[0.98] transition-transform"
        >
          Let&apos;s go
        </button>
      </div>
